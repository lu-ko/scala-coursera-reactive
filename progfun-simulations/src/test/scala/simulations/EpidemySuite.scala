...
class EpidemySuite extends FunSuite {
  ...
  test("test random") {
    ...
    while (testMoves > 0) {
      var rnd = es.randomBelow(8)
      assert(0 <= rnd && rnd < 8, "New random number is out scope: " + rnd)
      testMoves = testMoves - 1;
    }
  }
  ...
  test("test move - foot") {
    ...
    print(" Start: [" + person.row + ", " + person.col + "]")

    var testMoves = 100

    while (testMoves > 0) {
      var oldRow = person.row
      var oldCol = person.col
      var newPos = person.getNewPositionsFoot
      person.setNewPosition(newPos(es.randomBelow(newPos.length)))

      print(" -> [" + person.row + ", " + person.col + "]")
      assert(0 <= person.row && person.row < es.SimConfig.roomRows, "New row is out scope: " + person.row)
      assert(0 <= person.col && person.col < es.SimConfig.roomColumns, "New col is out scope: " + person.col)
      assert((oldRow != person.row) || (oldCol != person.col), "New row or cal has to be different!")
      testMoves = testMoves - 1;
    }
  }
  ...
  test("test move - air") {
    ...
    while (testMoves > 0) {
      var oldRow = person.row
      var oldCol = person.col
      person.setNewPosition(person.getNewPositionAir)
